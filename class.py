...
def apply(img, t):
    ...
    if t == "CARTOON":
        ...
        c = cv2.bilateralFilter(img,9,72,75)
        return cv2.bitwise_and(c,c,mask = e)
    return img

cap = cv2.VideoCapture(0)
if not cap.isOpened(): print("Error, could not access webcam"); exit()
...
while True:
    ...
    if res.multi_hand_landmarks:
        ...
        draw.draw_landmarks(img,hands,H.HAND_CONNECTIONS)
        lm = hand.landmark
        ...
        if pinch and not pinch_on and now-lc>CAP:
            pinch_on = True;capture = True; lc = now
        if not pinch and pinch_on: pinch_on = False
        if not pinch: 
            t=next((k for k in pairs if abs (tx-tips[k][0])<TT and abs (ty-tips[k][0])<TT),None)
            if t and now-la>DEB:cur = pairs[t][st[t]]; st[t]^=1;la = now; print("filters:", cur)
